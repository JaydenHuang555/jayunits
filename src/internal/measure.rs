use crate::internal::unit::Unit;

pub trait Measure<Num: crate::num::NumLike, U: Unit>: Clone {
    fn set_base(&mut self, base: Num);
    fn get_base(&self) -> Num;

    fn set(&mut self, value: Num, unit: &U) -> &Self {
        self.set_base(unit.to_base(value));
        self
    }

    fn to(&self, unit: &U) -> Num {
        unit.from_base(self.get_base())
    }
}

#[macro_export]
macro_rules! jayunits_measure_factory_build_impl {
    ($($t:ident, $u:ident),*) => {
        $(
            impl<Num> $t<Num> where Num: crate::num::NumLike {

                pub fn from(value: Num, u: &$u) -> Self {
                    Self {
                        base: u.to_base(value)
                    }
                }

                pub fn new() -> Self {
                    Self {
                        base: Num::from_f64(0.0)
                    }
                }

            }

            impl<Num> crate::internal::measure::Measure<Num, $u> for $t<Num> where Num: crate::num::NumLike {

                fn get_base(&self) -> Num {
                    self.base.clone()
                }

                fn set_base(&mut self, base: Num) {
                    self.base = base;
                }

            }


        )*
    };
}

#[macro_export]
macro_rules! jayunits_measure_factory_build_traits {
    ($($t:ident),*) => {
        $(

            impl<Num> Default for $t<Num> where Num: crate::num::NumLike {
                fn default() -> Self {
                    Self::new()
                }
            }

            impl<Num> std::ops::Add for $t<Num> where Num: crate::num::NumLike,
            {
                type Output = Self;

                fn add(self, other: Self) -> Self {
                    let base_param  = self.get_base() + other.get_base();
                    let mut ret = Self::default();
                    ret.set_base(base_param);
                    ret
                }
            }

            impl<Num> std::ops::Sub for $t<Num> where Num: crate::num::NumLike, {
                type Output = Self;

                fn sub(self, other: Self) -> Self {
                    let base_param = self.get_base() - other.get_base();
                    let mut ret = Self::default();
                    ret.set_base(base_param);
                    ret
                }
            }

            impl<Num> std::ops::Mul for $t<Num> where Num: crate::num::NumLike, {
                type Output = Self;

                fn mul(self, other: Self) -> Self {
                    let base_param = self.get_base() * other.get_base();
                    let mut ret = Self::default();
                    ret.set_base(base_param);
                    ret
                }
            }

            impl<Num> std::ops::Div for $t<Num> where Num: crate::num::NumLike, {
                type Output = Self;

                fn div(self, other: Self) -> Self {
                    let base_param = self.get_base() / other.get_base();
                    let mut ret = Self::default();
                    ret.set_base(base_param);
                    ret
                }
            }

            impl<Num> std::cmp::PartialEq for $t<Num> where Num: crate::num::NumLike {

                fn eq(&self, other: &Self) -> bool {
                    self.get_base() == other.get_base()
                }

            }

            impl<Num> std::fmt::Debug for $t<Num> where Num: crate::num::NumLike {

                fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                    f.debug_struct("Measure")
                        .field("base", &self.base)
                    .finish()
                }

            }

            impl<Num> Clone for $t<Num> where Num: crate::num::NumLike {

                fn clone(&self) -> Self {
                    Self {
                        base: self.base.clone()
                    }
                }

            }

            impl<Num> Copy for $t<Num> where Num: crate::num::NumLike {}

        )*
    };
}
